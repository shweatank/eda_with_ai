...
def _netlist_checks() -> list[str]:
    ...
    checks = []
    required_terms = ["module simple_ram", "posedge clk", "memory", "read_data"]
    for term in required_terms:
        if term not in text:
            checks.append(f"netlist is missing expected term: {term}")
    if not checks:
        checks.append("netlist contains the expected RAM module and clocked logic")
    return checks
...
def _ask_ollama(evidence: dict[str, object]) -> dict[str, str]:
    ...
    prompt = (
        "Review this 256-word, 8-bit RAM evidence. Return exactly "
        "{\"verdict\":\"PASS\"} or {\"verdict\":\"FAIL\"}. "
        "No explanation outside JSON.\n\n"
        + json.dumps({key: value for key, value in evidence.items() if key != "netlist"})
    )
    payload = {
        "model": model,
        "prompt": prompt,
