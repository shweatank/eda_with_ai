...
app = FastAPI(title="FSM Verification & Synthesis API")

BASE_DIR = Path(__file__).resolve().parent
VCD_FILE = BASE_DIR / "fsm.vcd"
SV_FILE = BASE_DIR / "fsm.sv"
NETLIST_FILE = BASE_DIR / "fsm_netlist.v"
TOP_MODULE = "fsm"
...
def root():
    return {
        ...
        "note": (
            "FSM lives in fsm.sv / test_fsm.py. POST /simulate runs cocotb. "
            "POST /synthesize runs Yosys and produces fsm_netlist.v. "
    ...
    if not VCD_FILE.exists():
        raise HTTPException(
            ...
            detail=f"make succeeded but {VCD_FILE.name} was not created. "
                   f"Check the $dumpfile name inside fsm.sv.",
        )
...
@app.post("/simulate")
def simulate():
    """Run the FSM cocotb testbench."""
    result = _run_make()
    return {
        "message": "Simulation complete for fsm",
...
def open_gtkwave():
    ...
    return {"message": "GTKWave launched for fsm.vcd"}
...
@app.post("/synthesize")
def synthesize():
    """Run read_verilog -> synth -top fsm -> write_verilog via Yosys."""
    result = _run_yosys()
    latch_detected = "latch" in result.stdout.lower()
    return {
        "message": "Synthesis complete for fsm",
...
def show_schematic():
    ...
    yosys_script = (
        f"read_verilog -noattr {NETLIST_FILE.name}; show -format dot -prefix fsm_show"
    )
    try:
        subprocess.Popen(
            ["yosys", "-p", yosys_script],
