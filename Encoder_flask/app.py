...
<html>
<head>
    <title>4-to-2 Encoder + Groq AI</title>
    <style>
...
<body>
    <h1>4-to-2 Binary Encoder + Groq AI</h1>
    <div class="card">
        <h3>Run Hardware Simulation & Stream AI</h3>
        <button onclick="runSimulation()">Simulate Encoder</button>
...
def simulate():
    ...
    subprocess.run("iverilog -g2012 -o sim.out encoder.sv", shell=True)
...
def stream_ai():
    def generate():
        try:
            client = Groq()
            prompt = f"""
            I simulated a 4-to-2 Binary Encoder in Verilog.
            Input lines (one-hot): {latest_metrics.get('input_a')}
            Encoded Binary Output: {latest_metrics.get('output_y')}
            Valid Flag: {latest_metrics.get('valid_flag')}
            Explain briefly how this combinational circuit maps a 4-line input to a 2-bit binary code.
            """
