...
ARTIFACTS = {
    "waveform": PROJECT_DIR / "parameters.vcd",
    "netlist": PROJECT_DIR / "parameters_netlist.v",
    "diagram": PROJECT_DIR / "parameters.dot",
    "image": PROJECT_DIR / "parameters.png",
}
...
def _netlist_checks() -> list[str]:
    ...
    checks = []
    for term in ["module counter", "parameter", "WIDTH", "count", "reset"]:
        if term not in text:
            checks.append(f"netlist is missing expected term: {term}")
    if not checks:
        checks.append("netlist contains the expected parameterized counter logic")
    return checks
...
def _ask_ollama(evidence: dict[str, object]) -> dict[str, str]:
    ...
    prompt = (
        "Review this parameterized counter evidence. Return exactly "
...
if __name__ == "__main__":
    app.run(host="127.0.0.1", port=int(os.getenv("PORT", "5005")), debug=False)
