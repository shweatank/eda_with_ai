...
ARTIFACTS = {
    "waveform": PROJECT_DIR / "dff_waveform.vcd",
    "netlist": PROJECT_DIR / "dflipflop_netlist.v",
    "diagram": PROJECT_DIR / "dff_design.dot",
    "image": PROJECT_DIR / "dff_design.png",
}
...
def _netlist_checks() -> list[str]:
    ...
    if not checks:
        checks.append("netlist contains the expected flip-flop sequential logic")
    return checks
...
def _ask_ollama(evidence: dict[str, object]) -> dict[str, str]:
    ...
    prompt = (
        ...
        "an asynchronous-reset D flip-flop. Return JSON only with keys "
