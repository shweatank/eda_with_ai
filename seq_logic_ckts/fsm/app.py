...
ARTIFACTS = {
    "waveform": PROJECT_DIR / "fsm_waveform.vcd",
    "netlist": PROJECT_DIR / "fsm_netlist.v",
    "diagram": PROJECT_DIR / "fsm.dot",
    "image": PROJECT_DIR / "fsm.png",
}
...
def _netlist_checks() -> list[str]:
    ...
    checks = []
    required_terms = ["module controller", "always", "current_state", "next_state", "case"]
    for term in required_terms:
        if term not in text:
            checks.append(f"netlist is missing expected term: {term}")
    if not checks:
        checks.append("netlist contains the expected FSM state machine logic")
    return checks
...
def _ask_ollama(evidence: dict[str, object]) -> dict[str, str]:
    ...
    prompt = (
        ...
        "Assess whether the synthesized netlist represents a valid finite-state machine controller "
        "with IDLE, LOAD, RUN, and DONE states and correct reset behavior. Return JSON only with keys "
