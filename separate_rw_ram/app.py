...
def _netlist_checks() -> list[str]:
    ...
    checks = []
    for term in ["module ram", "write_address", "read_address", "memory", "[15:0]"]:
        if term not in text:
            checks.append(f"netlist is missing expected term: {term}")
    if not checks:
        checks.append("netlist contains separate 8-bit read/write addresses and 16-bit data")
    return checks
...
def _ask_ollama(evidence: dict[str, object]) -> dict[str, str]:
    ...
    prompt = (
        "Review this 256-word, 16-bit RAM evidence with separate read and write addresses. "
...
if __name__ == "__main__":
    app.run(host="127.0.0.1", port=int(os.getenv("PORT", "5003")), debug=False)
