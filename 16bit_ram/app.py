...
ARTIFACTS = {
    "waveform": PROJECT_DIR / "ram16.vcd",
    "netlist": PROJECT_DIR / "ram16_netlist.v",
    "diagram": PROJECT_DIR / "ram16.dot",
    "image": PROJECT_DIR / "ram16.png",
}
...
def _netlist_checks() -> list[str]:
    ...
    checks = []
    for term in ["module simple_ram_16bit", "posedge clk", "memory", "[15:0]"]:
        if term not in text:
            checks.append(f"netlist is missing expected term: {term}")
    if not checks:
        checks.append("netlist contains the expected 16-bit RAM and clocked logic")
    return checks
...
def _ask_ollama(evidence: dict[str, object]) -> dict[str, str]:
    ...
    prompt = (
        "Review this 256-word, 16-bit RAM evidence. Return exactly "
