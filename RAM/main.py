...
app = FastAPI(title="Simple RAM Verification & Synthesis API")

BASE_DIR = Path(__file__).resolve().parent
VCD_FILE = BASE_DIR / "simple_ram.vcd"
SV_FILE = BASE_DIR / "simple_ram.sv"
NETLIST_FILE = BASE_DIR / "simple_ram_netlist.v"
TOP_MODULE = "simple_ram"
...
def root():
    return {
        ...
        "note": (
            "RAM lives in simple_ram.sv / test_simple_ram.py. POST /simulate runs cocotb. "
            "POST /synthesize runs Yosys and produces simple_ram_netlist.v. "
    ...
    if not VCD_FILE.exists():
        raise HTTPException(
            ...
            detail=f"make succeeded but {VCD_FILE.name} was not created. "
                   f"Check the $dumpfile name inside simple_ram.sv.",
        )
...
@app.post("/simulate")
def simulate():
    """Run the RAM cocotb testbench."""
    result = _run_make()
    return {
        "message": "Simulation complete for simple_ram",
...
def open_gtkwave():
    ...
    return {"message": "GTKWave launched for simple_ram.vcd"}
...
@app.post("/synthesize")
def synthesize():
    """Run read_verilog -> synth -top simple_ram -> write_verilog via Yosys."""
    result = _run_yosys()
    latch_detected = "latch" in result.stdout.lower()
    return {
        "message": "Synthesis complete for simple_ram",
...
def show_schematic():
    ...
    yosys_script = (
        f"read_verilog -noattr {NETLIST_FILE.name}; show -format dot -prefix simple_ram_show"
    )
    try:
        subprocess.Popen(
            ["yosys", "-p", yosys_script],
